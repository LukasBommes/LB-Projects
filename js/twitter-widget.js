...
(function( $, window, document, undefined ) {
	...
	$.fn.miniTwitter = function( options ) {
		...
		function fetchTweetsData ( object ) {
		    ...
		    obj.header = '<div class="mt_header"> <a '+rel()+' '+target()+' class="mt_user" href="'+obj.userUrl+'">'+obj.realName+'</a> <span class="mt_screen_name">@'+obj.screenName+'</span></div>';
		    obj.textTweet = '<div class="mt_text">'+obj.finalText+' <div class="time">'+obj.tweetTime+'</div></div>';
		...
		function display ( widget ) {
			fetch().done(function( res ) {
				...
				for(var i=0; i<tweets.length; i++) {
					$(widget).append(tweets[i].header+tweets[i].textTweet+tweets[i].footer);
				}
			}); 
		};
